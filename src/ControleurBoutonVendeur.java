...
Button button = (Button) (event.getSource());
    ...
    if (button.getText().contains("Ajouter un livre")) {
        this.MenuVendeur.fadeOut(button, () ->this.MenuVendeur.getRoot().setCenter(this.MenuVendeur.afficherPageAjouterLivre()));
        }

    if (button.getText().contains("Mise À jour Quantité Livre")) {
        this.MenuVendeur.fadeOut(button, () ->this.MenuVendeur.getRoot().setCenter(this.MenuVendeur.afficherPageMajQuantite()));
        }

    if (button.getText().contains("Disponibilité Livre")) {
        this.MenuVendeur.fadeOut(button, () ->this.MenuVendeur.getRoot().setCenter(this.MenuVendeur.afficherPageDispoLivres()));
        }
    
    if (button.getText().contains("Transférer un livre")) {
        this.MenuVendeur.fadeOut(button, () ->this.MenuVendeur.getRoot().setCenter(this.MenuVendeur.afficherPageTransfertLivre()));
        }
    if (button.getText().contains("Passer Commande pour un Client")) {
        this.MenuVendeur.fadeOut(button, () ->this.MenuVendeur.getRoot().setCenter(this.MenuVendeur.afficherPageCommandeClient()));
        }
